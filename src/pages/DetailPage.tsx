...
import { Clock, Shield, Heart, MessageSquare, Star, ArrowLeft, ImageIcon } from 'lucide-react';
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="md:col-span-2">
              ...
              <div className="bg-white rounded-lg shadow border border-slate-200 p-6 mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="bg-slate-50 rounded-lg p-4">
                    <p className="text-xs text-slate-600 mb-1 flex items-center gap-1"><Clock className="w-3 h-3" /> Umur</p>
              ...
              <div className="bg-white rounded-lg shadow border border-slate-200 p-6 mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {[
                    { key: 'phone', label: 'HP Terverifikasi', status: email.verifications.phone, verifiedText: 'Terverifikasi', unverifiedText: 'Belum' },
                    { key: 'recovery', label: 'Email Recovery', status: email.verifications.recovery, verifiedText: 'Aktif', unverifiedText: 'Tidak' },
                    { key: 'twoFa', label: '2FA', status: email.verifications.twoFa, verifiedText: 'Aktif', unverifiedText: 'Tidak' },
                  ].map(item => (
                    <div key={item.key} className={`flex items-center gap-3 p-4 rounded-lg ${item.status ? 'bg-green-50' : 'bg-slate-50'}`}>
                      <Shield className={`w-6 h-6 ${item.status ? 'text-green-600' : 'text-slate-400'}`} />
                      <div>
                        <p className="text-sm font-medium text-slate-900">{item.label}</p>
                        <p className={`text-xs ${item.status ? 'text-green-600' : 'text-slate-600'}`}>
                          {item.status ? `\u2713 ${item.verifiedText}` : `\u2717 ${item.unverifiedText}`}
                        </p>
                      </div>
                    </div>
                  ))}
              ...
              {email.imageCount > 0 && (
                <div className="bg-white rounded-lg shadow border border-slate-200 p-6">
                  ...
                  <p className="text-xs text-slate-600 mt-3">Gambar sensitif di-blur untuk privasi</p>
                </div>
              )}
            </div>
            ...
            <div className="md:col-span-1">
              ...
              <div className="bg-white rounded-lg shadow border border-slate-200 p-6 mb-6 sticky top-24">
                ...
                >
                  <Heart className={`w-5 h-5 ${isInWishlist(email.id) ? 'fill-red-500 text-red-500' : ''}`} />
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-sm font-medium text-slate-900">{email.sellerRating.toFixed(1)}</span>
                      <span className="text-sm text-slate-600">({email.sellerReviews} ulasan)</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <ul className="space-y-2 text-sm text-blue-900">
                  <li>&check; Akses instan setelah pembayaran</li>
                  <li>&check; Garansi {email.warranty}</li>
                  <li>&check; 100% uang kembali jika ada masalah</li>
                  <li>&check; Support 24/7</li>
                </ul>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-8">
            ...
            <label className="flex items-center gap-2 mb-6 cursor-pointer">
              ...
              <span className="text-sm text-slate-700">Saya setuju dengan syarat &amp; ketentuan</span>
            </label>
